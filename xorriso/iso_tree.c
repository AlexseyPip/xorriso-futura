...
int Xorriso_get_node_timestring(struct XorrisO *xorriso, void *in_node,
                                char *type, char time_string[41], int flag)
{
 int utc= 0, form= 3;
 time_t t;
 IsoNode *node;
 ...
 node= (IsoNode *) in_node;
 strcpy(time_string, "_time_type_not_found_");

 if(type[0] == 'a') {
   t= iso_node_get_atime(node);
 } else if(type[0] == 'c') {
   t= iso_node_get_ctime(node);
 } else if(type[0] == 'm') {
   t= iso_node_get_mtime(node);
 } else {
   return(0);
 }
 if(strchr(type, 'u') != NULL || strchr(type, 'U') != NULL)
   utc= 1;
 if(strchr(type, 's') != NULL)
   form= 1;
 Ftimetxt(t, time_string, (form << 1) | (utc << 8) | (1 << 9));
 return(1);
}
